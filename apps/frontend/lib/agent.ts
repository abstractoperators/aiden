...
async function getResource<T>(route: string, resource_id?: string): Promise<T> {
  try {
    ...
    if (!response.ok) throw new Error(`Failed to retrieve ${url}`);
...
async function getEnlightened(): Promise<Agent[]> {
  try {
    return Promise.all(
      (await getAgents()).map(async (apiAgent) => {
        const agent = {
          id: apiAgent.id,
...
export { getAgent, getEnlightened, getIncubating, getRuntime, getToken };

export type { Agent, ApiAgent, ApiToken, ApiRuntime };
